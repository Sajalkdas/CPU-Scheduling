...
timeline, processed, total_burst_time, total_time = fcfs(processes)
...
timeline_priority = priority_scheduling(processed)

# Print the timeline
print("\nTimeline (Priority-Based Scheduling):")
last_time = 0
for i, (time, event) in enumerate(timeline_priority):
    if event == "Idle":
...
average_waiting_time = sum(p['waiting_time'] for p in processed) / len(processed)
average_turnaround_time = sum(p['turnaround_time'] for p in processed) / len(processed)
